                        ...
                        <div className="flex flex-col flex-grow w-full max-w-sm">
                            <div className="flex flex-col mt-4 mb-6">
                                <FormWrapper onClick={HasProfile ? UpdateProfileOfUser : CreateProfileOfUser} buttonText={"Save"}>
                                    <FormInput label={t("Display name")} type="text" value={profile.name} onChange={(e) => { SetProfile({ ...profile, name: e.target.value }) }} />

                                    <FormInput label={t("Date of birth")} type="date" defaultValue={profile.birthdate.split('T')[0]} onChange={(e) => { SetProfile({ ...profile, birthdate: e.target.value }) }} />

                                    <FormSelect label={t("Gender")} value={profile.gender} onChange={(e) => { SetProfile({ ...profile, gender: e.target.value }) }}>
                                        <option value="Male">{t("Male")}</option>
                                        <option value="Female">{t("Female")}</option>
                                        <option value="Other">{t("Other")}</option>
                                    </FormSelect>

                                    <FormSelect label={t("Language")} value={profile.language} onChange={(e) => { SetProfile({ ...profile, language: e.target.value }) }}>
                                        <option value="en-US">{t("English")}</option>
                                        <option value="nl-NL">{t("Dutch")}</option>
                                    </FormSelect>

                                    <FormTags label={t("Interests")} childOnClickEvent={removeInterest} tagList={profile?.interests} onClick={addNewInterest} value={newInterest} onChange={(e) => { setNewInterest(e.target.value) }} />
                                </FormWrapper>

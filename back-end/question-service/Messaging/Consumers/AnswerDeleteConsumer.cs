            ...
            consumer.Received += async (sender, args) =>
            {
                if (args.BasicProperties.Headers.TryGetValue("MessageType", out var objValue) && objValue is byte[] valueAsBytes)
                {
                    ...
                    if (messageType == nameof(DeleteAnswerEvent))
                    {
                        ...
                        var message = JsonSerializer.Deserialize<DeleteAnswerEvent>(content, new JsonSerializerOptions { 
                            PropertyNamingPolicy = JsonNamingPolicy.CamelCase
                        });

                        await _service.DecrementNumberOfQuestionsAsync(new ObjectId(message.QuestionId));
